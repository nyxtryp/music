...
export default function AdminPanel() {
  ...
  const changePassword = async e => {
    ...
    try {
      const r = await fetch('/api/admin/password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminKey: currentPassword, newKey: newPassword })
      })
    ...
    try {
      for (let i = 0; i < selected.length; i += 1) {
        ...
        if (active !== 'photos') {
          ...
          await uploadToStorage(original, active, name, key, percentage => {
            setProgress({
            ...
            setStatus(`${i + 1}/${selected.length} · ЗАГРУЗКА ${percentage}%`)
          })
        ...
        await uploadToStorage(file, active, name, key, percentage => {
          setProgress({
            total: selected.length,
            current: i + 1,
            step: 'send',
      ...
      setProgress({
        ...
        step: 'done',
        name: selected[selected.length - 1].name
      })
      await load()
  ...
  const importFromUrl = async () => {
    const sourceUrl = window.prompt('ВСТАВЬТЕ URL ФАЙЛА')
    if (!sourceUrl?.trim()) return

    setBusy(true)
    setError('')
    setStatus('СКАЧИВАНИЕ ПО URL…')
    setProgress(null)
    ...
    try {
      const r = await fetch('/api/import-url', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: sourceUrl.trim(), type: active, adminKey: key })
      })
      const data = await r.json().catch(() => ({}))
      if (!r.ok) throw new Error(data.error || `Ошибка ${r.status}`)
      await load()
      setStatus(`ГОТОВО · ${data.name || 'ФАЙЛ СОХРАНЁН'}`)
    } catch (e) {
      setError(e.message || 'Ошибка загрузки по URL')
      setStatus('ОШИБКА ЗАГРУЗКИ')
    } finally {
      setBusy(false)
    }
  ...
  useEffect(() => {
    ...
    ;(files.photos || []).forEach(name => {
      next[name] = `/media/photos/${encodeURIComponent(name)}`
    })
    setPreviewUrls(next)
    return undefined
  }, [active, files.photos])
  ...
  <button className="plain" disabled={busy} onClick={() => { setPasswordModal(true); setError(''); setStatus('') }}>PASSWORD</button>
  <button className="plain exit" disabled={busy} onClick={logout}>EXIT <span>↗</span></button>
    ...
    <section className="toolbar">
      ...
      <button className="add" disabled={busy} onClick={importFromUrl}><span>↗</span>ADD BY URL</button>
    ...
    {passwordModal && <div className="modal-backdrop">
      <form className="modal" onSubmit={changePassword}>
        <div className="kicker">SECURITY</div>
        <h2>CHANGE PASSWORD</h2>
        <input autoFocus type="password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} placeholder="CURRENT PASSWORD"/>
        <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="NEW PASSWORD" style={{marginTop:'7px'}}/>
        <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="CONFIRM PASSWORD" style={{marginTop:'7px'}}/>
        {error && <p className="err">{error}</p>}
        <div className="modal-actions">
          <button type="button" className="plain" disabled={busy} onClick={() => { setPasswordModal(false); setCurrentPassword(''); setNewPassword(''); setConfirmPassword(''); setError('') }}>CANCEL</button>
          <button type="submit" disabled={busy}>{busy ? 'SAVING…' : 'CHANGE'}</button>
        </div>
      </form>
    </div>}
    ...
    <footer><span>{activeCount} FILES</span><span>VDS STORAGE</span><span>VIA NYXTRYP ADMIN</span></footer>
  </main></div>
}

const uploadToStorage = (file, type, name, adminKey, onProgress) => new Promise((resolve, reject) => {
  const xhr = new XMLHttpRequest()
  xhr.open('POST', `/api/upload?type=${encodeURIComponent(type)}&name=${encodeURIComponent(name)}`)
